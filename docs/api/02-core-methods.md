...
| `platform` | `string\|object` | `'sfcc'` | Platform provider. Built-in: `'sfcc'`. Pass an object for custom providers. |
...
| `commerceActions` | `object` | `undefined` | Regexes to override provider defaults for commerce actions. |
| `pageTypes` | `object` | `undefined` | Regexes to override provider defaults for page types. |
| `endpointFilter` | `RegExp` | `undefined` | Regex to filter which fetch/XHR calls are monitored. Overrides provider default. |
