...
import { Sanitizers, createSanitizer } from '../../src/utils/sanitizers.js';
...
describe('Sanitizers', () => {
    ...
    describe('sanitizeMessage', () => {
        ...
        it('should convert non-strings to strings', () => {
            expect(sanitizer.sanitizeMessage(12345)).toBe("12345");
            expect(sanitizer.sanitizeMessage(true)).toBe("true");
            expect(sanitizer.sanitizeMessage({})).toBe("[object Object]");
        });

        it('should redact email addresses', () => {
            const msg1 = "Contact me at user@example.com for info.";
            expect(sanitizer.sanitizeMessage(msg1)).toBe("Contact me at [EMAIL_REDACTED] for info.");

            const msg2 = "My emails are a.b@c.co.uk and test+123@gmail.com";
            expect(sanitizer.sanitizeMessage(msg2)).toBe("My emails are [EMAIL_REDACTED] and [EMAIL_REDACTED]");
        });

        it('should redact credit card numbers', () => {
            const msg1 = "Paid with 1234-5678-9012-3456";
            expect(sanitizer.sanitizeMessage(msg1)).toBe("Paid with [CARD_REDACTED]");

            const msg2 = "Cards: 1234 5678 9012 3456 and 1234567890123456";
            expect(sanitizer.sanitizeMessage(msg2)).toBe("Cards: [CARD_REDACTED] and [CARD_REDACTED]");
        });

        it('should redact phone numbers', () => {
            const msg1 = "Call 555-123-4567 or 555.987.6543";
            expect(sanitizer.sanitizeMessage(msg1)).toBe("Call [PHONE_REDACTED] or [PHONE_REDACTED]");
        });

        it('should redact generic tokens', () => {
            const msg1 = "Token: abcdefghijklmnopqrstuvwxyz123456";
            expect(sanitizer.sanitizeMessage(msg1)).toBe("Token: [TOKEN_REDACTED]");

            const msg2 = "Short token: abcdefg123";
            expect(sanitizer.sanitizeMessage(msg2)).toBe("Short token: abcdefg123");
        });

        it('should redact custom PII patterns', () => {
            sanitizer.registerPiiPatterns([
                { pattern: /SSN:\s*\d{3}-\d{2}-\d{4}/g, replacement: '[SSN_REDACTED]' }
            ]);
            const msg = "My SSN: 123-45-6789 is private.";
            expect(sanitizer.sanitizeMessage(msg)).toBe("My [SSN_REDACTED] is private.");
        });
        ...
        it('should reject ReDoS-vulnerable PII patterns', () => {
            ...
            expect(() => {
                sanitizer.registerPiiPatterns([
                    { pattern: /\d{3}-\d{4}/, replacement: '[REDACTED]' }
                ]);
            }).not.toThrow();
        });
        ...
        it('should truncate messages longer than 1000 characters', () => {
            ...
            const sanitized = sanitizer.sanitizeMessage(longMsg);
    ...
    describe('redactPII alias', () => {
        it('should alias sanitizeMessage', () => {
            ...
            expect(sanitizer.redactPII(msg)).toBe("Contact me at [EMAIL_REDACTED]");
        });
    });
